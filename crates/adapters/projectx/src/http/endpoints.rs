...
impl PxEndpoints {
    ...
    pub fn from_firm(firm: ProjectXTenant) -> Self {
        let firm_platform = firm.to_platform_name();
        if firm_platform == "demo" {
            return PxEndpoints {
        ...
        let api_base = format!("https://api.{firm_platform}.com",);
        let hub_base = format!("https://rtc.{firm_platform}.com",);
        PxEndpoints {
