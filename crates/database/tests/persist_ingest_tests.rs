...
fn temp_root() -> tempfile::TempDir { tempfile::tempdir().expect("tempdir") }
...
fn fake_candles(provider_s: &str, instrument: &Instrument, topic: Topic, day: NaiveDate, n: usize) -> Vec<CandleRow> {
    let res = match topic {
...
fn test_candles_and_bbo_monthly_paths_and_catalog() {
    ...
    let out_m1 = ingest_candles(&conn, &provider_kind, &instrument, market, Topic::Candles1m, &rows_m1, root, 9).unwrap();
    assert_eq!(out_m1.len(), 1);
    let expected_dir_m1 = partition_dir(root, provider_kind, market, &instrument, Topic::Candles1m, day.year() as u32);
    let expected_name_m1 = data_file_name(&instrument, Topic::Candles1m, NaiveDate::from_ymd_opt(day.year(), day.month(), 1).unwrap());
    assert_eq!(out_m1[0], expected_dir_m1.join(expected_name_m1));

    // BBO
    let base_dt = Utc.with_ymd_and_hms(day.year(), day.month(), day.day(), 10, 0, 0).unwrap();
    let base_ns = base_dt.timestamp_nanos_opt().unwrap();
    ...
    let out1 = ingest_bbo(&conn, &provider_kind, &instrument, market, Topic::Quotes, &b1, root, 9).unwrap();
    let out2 = ingest_bbo(&conn, &provider_kind, &instrument, market, Topic::Quotes, &b2, root, 9).unwrap();
    assert_eq!(out1, out2);
