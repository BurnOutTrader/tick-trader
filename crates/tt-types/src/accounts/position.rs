...
pub struct Lot { pub qty: i64, #[rkyv(with = DecimalDef)] pub price: Decimal }
...
impl PositionSegment {
    pub fn new(id: u64, side: Side, qty: i64, px: Decimal) -> Self {
        ...
        Self { id, side, open_qty: qty, net_qty: signed, open_vwap_px: px, realized_pnl: Decimal::ZERO, fees: Decimal::ZERO, lots_open: vec![Lot{qty, price: px}]} }
}
...
impl PositionLedger {
    pub fn new() -> Self { Self { segments: HashMap::new(), next_id: 1 } }

    pub fn apply_fill(&mut self, instrument: &Instrument, side: Side, qty: i64, price: Decimal, fee: Decimal) -> (i64, Decimal) {
        ...
        match entry {
            ...
            Some(mut seg) => {
                if seg.net_qty == 0 { // closed somehow, start new
                ...
                let seg_sign = match seg.side { Side::Buy => 1, Side::Sell => -1 } as i64;
                if seg_sign == side.sign() as i64 {
                    // same side: accumulate; update VWAP
                    let old_abs = seg.net_qty.abs();
                    let new_abs = old_abs + qty.abs();
                    if new_abs > 0 {
                        seg.open_vwap_px = ((seg.open_vwap_px * Decimal::from_i64(old_abs).unwrap()) + (price * Decimal::from_i64(qty.abs()).unwrap())) / Decimal::from_i64(new_abs).unwrap();
                    }
                    seg.net_qty += signed;
                    ...
                    let realized = Decimal::from_i64(close_qty).unwrap() * (price - seg.open_vwap_px) * Decimal::from_i32(seg_sign as i32).unwrap();
                    ...
                    if seg.net_qty == 0 {
                        ...
                        if remainder > 0 {
                            let seg2 = PositionSegment::new(self.alloc_id(), side, remainder, price);
                            self.segments.insert(instrument.clone(), seg2);
...
mod tests {
    ...
    fn accumulate_and_cross_flat() {
        ...
        // buy 2 @ 100
        l.apply_fill(&inst, Side::Buy, 2, Decimal::from_i32(100).unwrap(), Decimal::ZERO);
        // buy 1 @ 110 (vwap=103.33)
        l.apply_fill(&inst, Side::Buy, 1, Decimal::from_i32(110).unwrap(), Decimal::ZERO);
        // sell 4 @ 120 -> close 3 at 103.33, realized 16.67*3 = 50.01, remainder opens 1 short @120
        let (net_after, realized) = l.apply_fill(&inst, Side::Sell, 4, Decimal::from_i32(120).unwrap(), Decimal::ZERO);
    ...
    fn same_side_accumulates_and_updates_vwap() {
        ...
        let mut l = PositionLedger::new();
        l.apply_fill(&inst, Side::Buy, 1, Decimal::from_i32(100).unwrap(), Decimal::ZERO);
        l.apply_fill(&inst, Side::Buy, 3, Decimal::from_i32(110).unwrap(), Decimal::ZERO);
    ...
    fn close_to_flat_exact() {
        ...
        let mut l = PositionLedger::new();
        l.apply_fill(&inst, Side::Sell, 5, Decimal::from_i32(150).unwrap(), Decimal::ZERO);
        let (net_after, realized) = l.apply_fill(&inst, Side::Buy, 5, Decimal::from_i32(140).unwrap(), Decimal::ZERO);
    ...
    fn overshoot_reverses_and_starts_new_segment() {
        ...
        let (net_after, realized) = l.apply_fill(&inst, Side::Sell, 5, Decimal::from_i32(90).unwrap(), Decimal::ZERO);
    ...
    fn fees_accumulate_on_segment() {
        let inst = Instrument::try_from("ESZ5").unwrap();
        let mut l = PositionLedger::new();
        l.apply_fill(&inst, Side::Buy, 1, Decimal::from_i32(100).unwrap(), Decimal::from_f32(1.25).unwrap());
        l.apply_fill(&inst, Side::Buy, 1, Decimal::from_i32(101).unwrap(), Decimal::from_f32(0.75).unwrap());
        let seg = l.segments.get(&inst).unwrap();
