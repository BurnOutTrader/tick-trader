...
impl AccountActorHandle { pub async fn send(&self, ev: AccountEvent) { let _ = self.tx.send(ev).await; } }
...
impl AccountActor {
    ...
    fn apply(&mut self, ev: AccountEvent) {
        match ev {
            ...
            AccountEvent::Mark(m) => { self.marks.insert(m.instrument.clone(), m.mark_px); self.recompute_open_pnl(); }
        }
    }

    fn resolve_order_mut(&mut self, provider: &Option<ProviderOrderId>, client: &Option<ClientOrderId>) -> Option<&mut Order> {
        if let Some(p) = provider { if let Some(o) = self.orders_by_provider.get_mut(p) { return Some(o); } }
        if let Some(c) = client { if let Some(o) = self.orders_by_client.get_mut(c) { return Some(o); } }
        None
    }

    fn apply_order_event(&mut self, oe: OrderEvent) {
        match oe.kind {
            OrderEventKind::NewAck { instrument, side, qty } => {
                // create or update
                ...
                if ord.is_none() {
                    ...
                    if o.can_apply(oe.provider_seq, OrderState::Acknowledged) {
                        o.last_provider_seq = oe.provider_seq;
                        o.state = OrderState::Acknowledged;
                        if let Some(l) = oe.leaves_qty { o.leaves = l; }
                    }
                }
            }
            OrderEventKind::Replaced { new_qty } => {
                if let Some(o) = self.resolve_order_mut(&oe.provider_order_id, &oe.client_order_id) {
                    if o.can_apply(oe.provider_seq, o.state) {
                        o.version += 1;
                        if let Some(q) = new_qty { o.qty = q; o.leaves = q - o.cum_qty; }
                    }
                }
            }
    ...
    fn insert_order(&mut self, o: Order) {
        if let Some(p) = &o.provider_order_id { self.orders_by_provider.insert(p.clone(), o.clone()); }
        if let Some(c) = &o.client_order_id { self.orders_by_client.insert(c.clone(), o.clone()); }
    }

    fn apply_exec(&mut self, exe: ExecutionEvent) {
        // Dedupe
        if self.exec_by_id.contains(&exe.exec_id) { return; }
        ...
        if let Some(o) = self.resolve_order_mut(&exe.provider_order_id, &exe.client_order_id) {
            ...
            let total_px = o.avg_fill_px * Decimal::from_i64(o.cum_qty.abs()).unwrap_or(Decimal::ZERO)
                + exe.price * Decimal::from_i64(exe.qty.abs()).unwrap();
            o.cum_qty = new_cum;
            o.leaves = (o.qty - o.cum_qty).max(0);
            o.avg_fill_px = if new_cum > 0 { total_px / Decimal::from_i64(new_cum).unwrap() } else { o.avg_fill_px };
            o.state = if o.leaves == 0 { OrderState::Filled } else { OrderState::PartiallyFilled };
        }
        // Update position
        let (_net_after, realized_delta) = self.positions.apply_fill(&exe.instrument, exe.side, exe.qty.abs(), exe.price, exe.fee);
        if realized_delta != Decimal::ZERO { self.state.day_realized_pnl += realized_delta; }
    ...
    fn recompute_open_pnl(&mut self) {
        ...
        for (inst, seg) in self.positions.segments.iter() {
            if seg.net_qty == 0 { continue; }
            if let Some(mark) = self.marks.get(inst) {
                let sign = match seg.side { Side::Buy => 1, Side::Sell => -1 };
                open += Decimal::from_i64(seg.net_qty.abs()).unwrap() * (*mark - seg.open_vwap_px) * Decimal::from_i32(sign).unwrap();
            }
        }
...
mod tests {
    ...
    fn inst(code: &str) -> Instrument { Instrument::try_from(code).unwrap() }
    ...
    fn order_lifecycle_ack_replace_cancel() {
        ...
        actor.apply(AccountEvent::Order(OrderEvent{
            kind: OrderEventKind::NewAck { instrument: inst("ESZ5"), side: Side::Buy, qty: 10 },
    ...
    fn exec_updates_order_and_positions_and_dedupe() {
        ...
        actor.apply(AccountEvent::Order(OrderEvent{
            kind: OrderEventKind::NewAck { instrument: inst("MNQZ5"), side: Side::Buy, qty: 3 },
            provider_order_id: Some(prov.clone()),
        ...
        actor.apply(AccountEvent::Mark(MarkEvent{ instrument: inst("MNQZ5"), mark_px: Decimal::from_i32(105).unwrap(), ts_ns: 3 }));
    ...
    fn cancel_after_fill_with_no_seq_rules_overrides_in_current_impl() {
        ...
        actor.apply(AccountEvent::Order(OrderEvent{
            ...
            provider_order_id: Some(prov.clone()), client_order_id: None, provider_seq: Some(1), leaves_qty: Some(1), ts_ns: 1
        }));
        actor.apply(AccountEvent::Exec(ExecutionEvent{ exec_id: ExecId("E10".into()), provider_order_id: Some(prov.clone()), client_order_id: None, side: Side::Buy, qty: 1, price: Decimal::from_i32(100).unwrap(), fee: Decimal::ZERO, ts_ns: 2, provider_seq: Some(2), instrument: inst("ESZ5") }));
