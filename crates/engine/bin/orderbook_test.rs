...
impl Strategy for TestLiveOrdersStrategy {
    ...
    async fn on_stop(&mut self) { info!("test strategy stop"); }
    ...
    async fn on_orders_batch(&mut self, b: wire::OrdersBatch) { info!("orders: {} updates", b.orders.len()); }
    async fn on_positions_batch(&mut self, _b: wire::PositionsBatch) {}
    async fn on_account_delta(&mut self, accounts: Vec<AccountDelta>) { for a in accounts { info!("account: can_trade={} eq={} ts={}", a.can_trade, a.equity, a.ts_ns); } }
    async fn on_subscribe(&mut self, instrument: Instrument, data_topic: tt_engine::engine::DataTopic, success: bool) {
        info!("Subscribed to {} on {:?}: success={}", instrument, data_topic, success);
...
async fn main() -> anyhow::Result<()> {
    ...
    let strategy_placeholder = Arc::new(Mutex::new(TestLiveOrdersStrategy::new(StrategyConfig { key: key.clone(), instrument: instrument.clone(), account_name: account_name.clone() })));
    let _handle = engine.start(strategy_placeholder.clone()).await?;

    // Subscribe to account streams
    engine.initialize_account_names(provider, vec![account_name]).await?;
