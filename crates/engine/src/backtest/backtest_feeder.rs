...
impl BacktestFeeder {
    ...
    pub fn start_with_db(
        ...
        std::mem::drop(tokio::spawn(async move {
            ...
            // Await-driven scheduling: stash a single awaited request here to be processed first
            let mut pending_req: Option<tt_types::wire::Request> = None;

            // Main loop: interleave handling of requests with emitting events in time order
            loop {
                let bus = BUS_CLIENT.get().unwrap();
                // 1) Drain any pending requests: process any awaited one first, then drain non-blocking
                let mut first_req = pending_req.take();
                loop {
                    let req = if let Some(r) = first_req.take() {
                        Some(r)
                    } else {
                        req_rx.try_recv().ok()
                    };
                        ...
                        match req {
                            Request::SubscribeAccount(req) => {
                                if !PORTFOLIOS.contains_key(&req.key) {
                                    ...
                                    if let Ok(map) =
                                        tt_database::queries::get_contracts_map(&conn, provider)
                                            .await
                                    {
                                    ...
                                    for pending in pre_wm_queue.drain(..) {
                                        match pending {
                                            _Req::PlaceOrder(mut spec) => {
                                                ...
                                                let acct_name =
                                                    spec.account_key.account_name.clone();
                                                ...
                                                let (engine_order_id, user_tag) =
                                                    if let Some(tag) = &spec.custom_tag {
                                                ...
                                                let provider_order_id = ProviderOrderId(format!(
                                                    "bt-{}",
                                                    engine_order_id
                                                ));
                                                ...
                                                let normalized_qty =
                                                    spec.side.normalize_qty(spec.qty);
                                                if normalized_qty == 0 {
                                                    reject_reason = Some(
                                                ...
                                                let ack_at =
                                                    ack_base + to_chrono(lat.submit_to_ack());
                                                let fill_at =
                                        ...
                                        && let Some(ks) = keys.get_mut(&(topic, sk.clone()))
                                    {
                                        if let Some(mut vec) = ks.buf.remove(&t) {
                                            ...
                                            if let Some(ref clock) = clock {
                                                let ns =
                                                    t.timestamp_nanos_opt().unwrap_or(0) as u64;
                                                clock.advance_to_at_least(ns);
                                                clock.bump_ns(1);
                                            }
                                            for item in vec.drain(..) {
                                                match &item {
                                                    tt_database::queries::TopicDataEnum::Tick(
                                                        tk,
                                                    ) => {
                                                    ...
                                                    tt_database::queries::TopicDataEnum::Bbo(
                                                        bbo,
                                                    ) => {
                                                        let key = SymbolKey::new(
                                                    ...
                                                    tt_database::queries::TopicDataEnum::Candle(
                                                        c,
                                                    ) => {
                                                        let key = SymbolKey::new(
                                                            c.instrument.clone(),
                                                    ...
                                                    tt_database::queries::TopicDataEnum::Mbp10(
                                                        m,
                                                    ) => {
                                                        let key = SymbolKey::new(
                                                            m.instrument.clone(),
                                ...
                                if let Some(liq_window) = cfg.flatten_before_close {
                                    ...
                                    for key in keys {
                                        if let Some(pd) = positions_ledger.get(&key) {
                                            ...
                                            if let Some(close) =
                                                cal_model.next_close_after(&instr, now)
                                            {
                                                ...
                                                if until_close <= liq_window {
                                                    ...
                                                    let sym_key =
                                                        SymbolKey::new(instr.clone(), key.0);
                                                    let ref_px =
                                                        if let Some(m) = marks.get(&sym_key) {
                                                            m.ref_px()
                                                        } else {
                                                            Decimal::ZERO
                                                        };
                                                    ...
                                                    while let Some(lot) = match cfg.matching_policy
                                                    {
                                                        ...
                                                        let pnl =
                                                            ticks * value_per_tick * matched_dec;
                                                        // Record trade
                                                        closed_trades.push(tt_types::wire::Trade {
                                                            id: EngineUuid::new(),
                                                            provider: key.0,
                                                        ...
                                                        entry.time = now;
                                                        updated_accounts
                                                            .push((key.0, key.1.clone()));
                                                    }
                                                    // Update position to flat
                                                    if let Some(pdm) =
                                                        positions_ledger.get_mut(&key)
                                                    {
                                ...
                                match spec.order_type {
                                    ...
                                    tt_types::wire::OrderType::StopLimit => {
                                        if spec.stop_price.is_none() || spec.limit_price.is_none() {
                                            reject = true;
                                            msg = Some(
                                                "Stop price is none or Limit price is none"
                                                    .to_string(),
                                            )
                                        }
                                    }
                ...
                if !did_work {
                    ...
                    // idle wait: await next request to avoid busy loop and yield cooperatively
