...
#[tokio::test]
async fn candles_to_candles_consolidator_only_from_lower_resolution() {
    let (req_tx, mut _req_rx) = tokio::sync::mpsc::channel::<tt_types::wire::Request>(8);
    let bus = ClientMessageBus::new_with_transport(req_tx);
    ...
    let provider = ProviderKind::ProjectX(ProjectXTenant::Demo);
    let instr = Instrument::from_str("ES.Z25").unwrap();
    let key = SymbolKey::new(instr.clone(), provider);

    // Register a 5m-from-1m consolidator
    let cons = tt_types::consolidators::CandlesToCandlesConsolidator::new(
        Resolution::Minutes(5),
        "ES".to_string(),
        None,
        instr.clone(),
    );
    handle.add_consolidator(DataTopic::Candles1m, key.clone(), Box::new(cons));

    // Feed 5 one-minute bars -> expect one consolidated 5m bar callback
    let start = chrono::Utc::now();
    let mut bars_1m = Vec::new();
    for i in 0..5 {
        let t0 = start + chrono::Duration::minutes(i);
        let bar = Candle {
            symbol: "ES".into(),
            instrument: instr.clone(),
            time_start: t0,
            time_end: t0 + chrono::Duration::minutes(1) - chrono::Duration::nanoseconds(1),
            open: (100 + i).into(),
            high: (101 + i).into(),
            low: (99 + i).into(),
            close: (100 + i).into(),
    ...
    let _ = bus
        .route_response(Response::BarBatch(BarBatch {
    ...
    let count_after_1m = *probe.on_bar_count.lock().unwrap();
    assert!(
        count_after_1m >= 6,
        "should see original 5 + at least 1 consolidated bar"
    );

    // Now send an equal-resolution bar (5m) — the consolidator should ignore it (no extra on_bar)
    let bar_5m = Candle {
        symbol: "ES".into(),
        instrument: instr.clone(),
        time_start: start,
        time_end: start + chrono::Duration::minutes(5) - chrono::Duration::nanoseconds(1),
        open: 120.into(),
        high: 121.into(),
        low: 119.into(),
        close: 120.into(),
    ...
    let _ = bus
        .route_response(Response::BarBatch(BarBatch {
            topic: Topic::Candles1m, // engine maps topic from incoming resolution; test path: we leave consolidator registered on Candles1m only
            seq: 2,
            bars: vec![bar_5m],
            provider_kind: provider,
        }))
    ...
    sleep(Duration::from_millis(20)).await;
    let count_after_5m = *probe.on_bar_count.lock().unwrap();
    // No guarantee on original callback count increments (engine still forwards incoming bar),
    // but consolidator must not add extra consolidated outputs on equal/coarser frames.
    assert!(
        count_after_5m - count_after_1m <= 1,
        "equal/coarser res should not produce additional consolidated bars"
    );
